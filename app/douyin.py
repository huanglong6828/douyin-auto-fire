...
class DouyinChat:
    ...
    async def open_target(self, name: str, remark: str | None = None, retries: int = 1) -> None:
        last_error: Exception | None = None
        for attempt in range(retries + 1):
            try:
                await self._open_target_once(name, remark)
                return
    ...
    async def _open_target_once(self, name: str, remark: str | None = None) -> None:
        candidates = _search_candidates(name, remark)
        search = await first_visible(self.page, SEARCH_INPUTS, self.timeout_ms)
        await search.click()
        await search.fill("")
        # 备注更精确、搜索结果更少，优先使用备注；未设备注时回退昵称。
        await search.fill(candidates[0])
        await self.page.wait_for_timeout(1_500)

        result = await self._search_result(candidates)
        if result is None:
        ...
        await self._confirm_opened(candidates, name)

    async def _search_result(self, names: tuple[str, ...]) -> Locator | None:
    ...
    async def _confirm_opened(self, names: tuple[str, ...], primary: str, timeout_ms: int | None = None) -> None:
        timeout = timeout_ms if timeout_ms is not None else self.confirm_timeout_ms
        deadline = asyncio.get_running_loop().time() + timeout / 1000
        while True:
            last_error = await self._chat_open_error(names, primary)
    ...
    async def _chat_open_error(self, names: tuple[str, ...], primary: str) -> PageOperationError | None:
        ...
        if composer_visible:
            ...
            try:
                ...
                body_text = ""
            if any(name in body_text for name in names):
                return None
            for name in names:
        ...
        return PageOperationError(
            f"点击搜索结果后无法确认聊天已打开: {primary}（输入框: {'有' if composer_visible else '无'}）"
        )
...
def _search_candidates(name: str, remark: str | None) -> tuple[str, ...]:
    """构造搜索/匹配候选名列表。备注优先（更精确、结果更少），昵称兜底。"""
